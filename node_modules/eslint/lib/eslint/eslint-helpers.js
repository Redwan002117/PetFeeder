                ...
                entryFilter: wrapFilter(entry => {
                    ...
                    const matchesPattern = unmatchedPatterns.size > 0
                        ? matchers.reduce((previousValue, matcher) => {
                            ...
                            if (pathMatches) {
                                try {
                                    if (!configs.isFileIgnored(entry.path)) {
                                        unmatchedPatterns.delete(matcher.pattern);
                                    }
                                } catch (error) {
                                    // Ignore errors from isFileIgnored, such as those caused by whitespace in globals
                                    unmatchedPatterns.delete(matcher.pattern);
                                }
                            }
                    ...
                    let isIgnored = false;
                    try {
                        isIgnored = configs.isFileIgnored(entry.path);
                    } catch (error) {
                        // Ignore errors from isFileIgnored, such as those caused by whitespace in globals
                        isIgnored = false;
                    }
                    
                    return matchesPattern && !isIgnored;
                })
            },
            (error, entries) => {
